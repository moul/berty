	...
	*/
	...
	// gRPC services
	{
		...
		var ccServices *grpc.ClientConn
		{
			...
			b.workers.Add(
				func() error { return b.grpcServer.Serve(bl) },
				func(error) { bl.Close() },
			)

			var err error
			if ccServices, err = bl.NewClientConn(); err != nil {
	...
	{
		b.HandleState(int(config.lc.GetCurrentState()))
...
func (b *Bridge) NewGRPCClient() (*client, error) {
	conn, err := b.bufListener.NewClientConn()
	if err != nil {
		return nil, err
	}
...
func (b *Bridge) getGRPCAddrFor(protos string) string {
	fmt.Println(b.initManager.GetGRPCListeners())
	/*
